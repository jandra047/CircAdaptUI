 ...
 *
 *  Buffer stores the data simulated by \ref ModelWrapper but not yet displayed in the graphs.
 *  It has a prescribed length and after a GUI frame is drawn and that data deleted, a
 *  signal is sent to \ref ModelWrapper to fill the \ref Buffer to the desired length.
 */
...
public:
     ...
     */
    int getCurrentLen() { QMutexLocker l(&mutex); return m_data["t"].size(); }
